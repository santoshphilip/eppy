...
walls = dt[wallkey]
surfaces = [Bunch(list(zip(wall_fields, wall))) for wall in walls]
...
for i, key in enumerate(dtls):
    ...
    tmplst = []
    bunchdt[key] = [Bunch(list(zip(fields, obj))) for obj in dt[key]]
    
# push bunchdt into data
for i, key in enumerate(dtls):
    fnames = [fd['field'][0] for fd in commdct[i] if 'field' in fd]
