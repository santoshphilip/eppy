...
def flattenkey(btabledct):
    ...
    decdct= {}
    for boldkey in list(btabledct.keys()):
        for gridkey in list(btabledct[boldkey].keys()):
            lst = [boldkey] + list(gridkey)
            newkey = tuple(lst)
            decdct[newkey] = btabledct[boldkey][gridkey]
    return decdct    

def bigsmallequal(xxx_todo_changeme):
    """test if the value is big, small or equal, or None"""
    (value, isdiff) = xxx_todo_changeme
    if isdiff == True:
        return 'big'
    if isdiff == False:
        try:
...
def matrix2table3(table, mat, theformat='%s'):
    ...
    for old, new in zip(alltds, flat):
        ...
        try:
            ...
            old.contents[0].replaceWith(str(newval))
        except IndexError as e: # no contents to replace
            pass 
    return thesum           
...
def error2table(table):
    ...
    for td in alltds:
        td['class'] = valuetype
        try:
            td.contents[0].replaceWith(str(valuetype))
        except IndexError as e: # no contents to replace
            pass 
