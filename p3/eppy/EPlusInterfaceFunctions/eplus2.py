...
class ordereddict(object):
	...
	def add(self,name,content,warn=1):
		...
		if warn:
			if name in self.adict:
				raise ValueError('%s already exists in the ordereddict' %(repr(name),))
		self.adict[name]=content
		try:
	...
	def update(self,name,content):
		if not(name in self.adict):
			raise ValueError('%s does not exists in the ordereddict' %(repr(name),))
		self.adict[name]=content
...
class nocomment(object):
	...
	def tofile(self,filename):
		mylib2.str2file(filename,repr(self))
